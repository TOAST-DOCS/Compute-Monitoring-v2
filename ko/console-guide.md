...
| 추가 알람 | 설명 |
| ----------| -----|
| 10분 지속 알람   | 최초 알람 발송 후, 해당 상태가 10분 이상 지속되는 경우 |
| 한시간 지속 알람 | 최초 알람 발송 후, 해당 상태가 한시간(60분) 이상 지속되는 경우 |
| 하루 지속 알람   | 최초 알람 발송 후, 해당 상태가 하루(1440분) 이상 지속되는 경우 |
...
> [주의]
> 이로 인해 발생할 수 있는 불이익에 대해 Toast 클라우드는 책임을 지지 않습니다.

에이전트는 5초 주기로 데이터를 수집 및 전송합니다.
만약 에이전트가 구동 중임에도 그래프가 정상적으로 표시되지 않는 경우에는 Toast 고객센터로 문의주시기 바랍니다.
