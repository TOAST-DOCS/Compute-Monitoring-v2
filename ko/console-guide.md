...
|메모리 사용률  | pused: 메모리 사용률(%)<br>swappused: 스와프 사용률(%)<br><br>*[참고] Linux 인스턴스의 메모리 사용률 계산 시 버퍼 및 캐시 영역은 사용 가능한 영역으로 간주하여 사용량에 포함하지 않습니다.* |
...
>Memory Usage 그래프: 각 인스턴스의 memory 사용률(pused)만 제공
