...
| 그래프 끊김 현상 | 에이전트의 지표 수집 기능 혹은 서버로의 지표 전송에 문제가 생기는 경우 그래프 조회 시 라인 끊김 현상이 발생 할 수 있습니다.<br>그와 같은 현상은 다음과 같은 상황에서 발생할 수 있습니다.<br> 1. 에이전트 프로세스 종료 후 재시작 시 그동안의 지표는 수집 안됨<br>2. 인스턴스에 floating ip 연결 혹은 해제 시 패킷 전송 경로 변경 작업이 발생하며, 그동안 지표 데이터 전송이 불가능함<br> 3. 시스템 과부하(cpu 및 disk io 등)로 인해 일반적인 프로세스의 정상 동작이 어려운 경우, 에이전트 프로세스 역시 정상적인 지표 수집이 불가능함 | 
