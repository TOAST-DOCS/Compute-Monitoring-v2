...
|메모리 사용률  | used: 메모리 사용률(%)<br>swapused: 스와프 사용률(%)<br><br>*[참고] Linux 인스턴스의 메모리 사용률 계산 시 버퍼 및 캐시 영역은 사용 가능한 영역으로 간주하여 사용량에 포함하지 않습니다.*<br>*[참고] Windows 인스턴스의 스와프 사용률은 페이지 파일 사용량을 기반으로 계산되며, 물리 메모리 크기는 고려되지 않습니다.* |
