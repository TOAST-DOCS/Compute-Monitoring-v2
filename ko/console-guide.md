...
>[참고]
>다음의 두 그래프에서 보여지는 지표는 모니터링 탭에서와는 달리 일부 지표만 제공됩니다.<br>
>CPU Usage 그래프: 각 인스턴스에 대해 total 사용률만 제공
>Memory Usage 그래프: 각 인스턴스에 대해 memory 사용률만 제공
...
알람 정책은 인스턴스 상태 감시를 위한 각종 설정 내역을 담고 있으며, 이를 바탕으로 사용자는 인스턴스 상태에 따른 알람을 받을 수 있습니다.
...
> 임의의 알람 이벤트에 대한 최초 알람 발송 후, 아래와 같이 최대 3회의 추가 알람이 발송될 수 있습니다.<br>
> 1. 10분 지속 알람: 최초 알람 발송 후, 해당 상태가 10분 이상 지속되는 경우<br> 
> 2. 한시간 지속 알람: 최초 알람 발송 후, 해당 상태가 한시간(60분) 이상 지속되는 경우<br> 
> 3. 하루 지속 알람: 최초 알람 발송 후, 해당 상태가 하루(1440분) 이상 지속되는 경우 

Monitoring v2 서비스에서는 기본적으로 `Default 정책`을 제공하며, 이 정책은 수정 가능하나 삭제는 불가능합니다.
또한 사용자에 의한 알람 정책의 생성, 수정 및 삭제 기능도 제공합니다.

>[참고]
>모니터링 서비스 등록(에이전트 최초 구동) 직후의 인스턴스는 Default 정책이 적용되며, 언제든지 사용자에 의해 다른 정책의 적용이 가능합니다.
...
각 알람 이력은 다음과 같은 항목으로 구성되어 있습니다.
...
알람 이력은 인스턴스 이름 혹은 IP 주소를 이용한 검색이 가능합니다.
또한 아래와 같이 원하는 시간대를 지정하여, 해당 시점의 알람 이력만을 조회할 수도 있습니다.
