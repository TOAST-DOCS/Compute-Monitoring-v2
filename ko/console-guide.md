...
|메모리 사용률  | used: 메모리 사용률(%)<br>swapused: 스와프 사용률(%)<br><br>*[참고] Linux 인스턴스의 메모리 사용률 계산 시 버퍼 및 캐시 영역은 사용 가능한 영역으로 간주하여 사용량에 포함하지 않습니다.*<br>*[참고] Windows 인스턴스의 스와프 사용률 계산 시 물리 메모리 크기는 고려하지 않고, 페이지 파일 사용량만을 이용합니다.* |
...
> [참고] 'inActive' 상태의 인스턴스
> 다음과 같은 인스턴스들은 인스턴스 목록에서 'inActive' 상태로 표시됩니다.
> 1. 에이전트 구동 이력이 있으나 현재 에이전트가 종료된 인스턴스
> 2. 에이전트 구동 이력이 있으나 현재 종료 상태의 인스턴스
> 3. 에이전트 구동 이력이 있으나 현재 에이전트가 삭제된 인스턴스
...
>[참고] 두개의 임계 수준(warn 및 fatal)을 모두 충족시키는 상황에서의 알람 메세지 관련
>warn 및 fatal 임계값을 모두 넘어선 상황이 발생한 경우, 각 임계 수준에 대한 알람 메세지가 독립적으로 발송됩니다.
>(warn 수준의 알람이 먼저 발생 후, fatal 수준으로 넘어간 경우도 포함)
>단, 이와 같은 상황에서의 OK 알람 메세지는 warn 수준의 알람 상태가 정상화되는 경우에만 발송됩니다.
>(즉, fatal 수준의 알람 상태만 해소되는 경우에는 이에 대한 OK 알람 메세지가 발송되지 않습니다.)

## 알람 이력 조회
**Alarm Logs** 탭에서 알람 이력을 조회할 수 있습니다.
각 알람 이력은 알람 상태가 최초로 감지된 시점에 생성됩니다.
>[참고]
> 하나의 알람 이력에 대한 알람 메세지(email 혹은 sms)는 해당 상태 지속 여부에 따라 최대 4회 발송될 수 있지만, 알람 이력은 알람 메세지 발송 회수와 관계없이 1개만 생성됩니다.
...
| 그래프 끊김 현상 | 에이전트의 지표 수집 기능 혹은 서버로의 지표 전송에 문제가 생기는 경우 그래프 조회 시 라인 끊김 현상이 발생할 수 있습니다.<br>그와 같은 현상은 다음과 같은 상황에서 발생합니다.<br> 1. 에이전트 프로세스 종료 후 재시작 시 그동안의 지표는 수집 안됨<br>2. 인스턴스에 floating ip 연결 혹은 해제 시 패킷 전송 경로 변경 작업이 발생하며, 그동안 지표 전송이 불가능함<br> 3. 시스템 과부하(cpu 및 disk io 등)로 인해 일반적인 프로세스의 정상 동작이 어려운 경우, 에이전트 프로세스 역시 정상적인 지표 수집이 불가능함 | 
